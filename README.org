...
#+END_SRC
** Local Installation via Operating System Packages
*** Debian package form
...
#+caption: (ABCL) Install the Quicklisp packages needed by =chute=
#+begin_src lisp
  (asdf:make :quicklisp-abcl)
  (asdf:make :chute)
#+end_src    

#+caption: Have ASDF build =chute=
#+begin_src lisp
  (time (asdf:make :chute))
#+end_src

   
#+caption: Have ASDF test =chute=
#+BEGIN_SRC lisp
(asdf:test-system :chute)
#+END_SRC    

* Implementation
=chute= provides both a client and a server implementation.  All
...
*** DONE [doc] configure an ANSI CL and Quicklisp installation
